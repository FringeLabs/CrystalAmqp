  ...
  AMQP::Client.start("") do |c|
    c.channel do |ch2|
      c.channel do |ch|
        ...
        ch.exchange_declare("data.gpp.processed", type: "direct")
        ch.queue_bind("local-crystal-processor", "data.gpp", "")
        ch.prefetch(count: 500) # alias for basic_qos

        # consuming
        q = ch.queue("local-crystal-processor")
        q.subscribe(no_ack: false, block: true) do |msg|
          ch2.basic_publish msg.body_io, exchange: "data.gpp.processed", routing_key: msg.routing_key
